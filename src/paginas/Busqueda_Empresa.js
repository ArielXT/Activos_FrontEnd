...
function Busqueda_Empresa() {
    ...
    const [tablasetEmpresa, setTablaEmpresa] = useState([]);
    const [busqueda, setBusqueda] = useState("");


    const peticionGet = async () => {
        await axios.get("http://127.0.0.1:8000/cye/empresa/")
            .then(response => {
                setEmpresa(response.data);
                setTablaEmpresa(response.data);
            }).catch(error => {
                console.log(error);
            })
    }
    ...
    const filtrar = (terminoBusqueda) => {
        var resultadosBusqueda = tablasetEmpresa.filter((elemento) => {
            if (elemento.id_empresa.toString().toLowerCase().includes(terminoBusqueda.toLowerCase())
...
export default Busqueda_Empresa;