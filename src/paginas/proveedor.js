...
import { faEdit, faTrashAlt, faTruck, faHome, faSearch } from '@fortawesome/free-solid-svg-icons';
import { Modal, ModalBody, ModalFooter, ModalHeader } from 'reactstrap';
import { NavLink, Link } from 'react-router-dom';
import { Navbar, Nav } from 'react-bootstrap';
const url = "http://127.0.0.1:8000/cye/proveedor/";
            ...
            <body className="back">
                <div className="tiledBackground">
                    ...
                    <div className="container">
                        ...
                        <div className="centro"><Link to={"/Busqueda_Proveedor/"} className="btn btn-warning">Hacer una Busqueda <FontAwesomeIcon icon={faSearch} /></Link></div>
                        <button type="button" className="btn btn-success" onClick={() => { this.setState({ form: null, tipoModal: 'insertar' }); this.Insertar() }}>Agregar Un Nuevo Proveedor <FontAwesomeIcon icon={faTruck} /></button>
                        ...
                        <table className="table table-condensed col-sm-8 text-center  table-dark table-striped table-bordered table-responsive" border="1">
                            ...
                            <tbody>{this.state.data.map(proveedor => {
                                return (
                                    <tr key={proveedor.id_proveedor}>
                                        ...
                                        <td>
                                            <button type="button" className="btn btn-warning mx-2" onClick={() => { this.seleccionarProveedor(proveedor); this.Insertar() }}>Editar <FontAwesomeIcon icon={faEdit} /></button>
                                            <button type="button" className="btn btn-danger" onClick={() => { this.seleccionarProveedor(proveedor); this.setState({ Eliminar: true }) }}>Eliminar <FontAwesomeIcon icon={faTrashAlt} /></button>
                                        </td>
                                    </tr>
                                )
                            })}
                        ...
                        <Modal isOpen={this.state.Insertar} className="my-modal">
                            ...
                            <ModalBody>
                                <div className="form-group">
                                    <label htmlFor="id_proveedor" style={{ color: "white" }}>ID Proveedor</label>
                        ...
                        <Modal isOpen={this.state.Eliminar}>
                            <ModalBody>
                                ¿Estás seguro que deseas eliminar a el Proveedor:  {form && form.nombre}?
                            </ModalBody>
                            <ModalFooter>
